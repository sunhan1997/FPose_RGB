...
class PoseRefinePredictor:
  def __init__(self,):
    ...
    # ############## sunhan train
    # self.model_rgb = RefineNet_RGB(cfg=self.cfg, c_in=3).cuda()

    # pretrained_weights = torch.load('/home/sunh/6D_ws/Fpose_rgb/weights/obj16_291*10/model_best.pth')  ## my weight
    # self.model_rgb.load_state_dict(pretrained_weights['model_state_dict'])
    ...
    # # 需要处理的权重
    # with torch.no_grad():
    #     pretrained_weights['encodeA.0.net.0.weight'] = pretrained_weights['encodeA.0.net.0.weight'][:,:3,:,:]
    # # 加载权重
    # self.model_rgb.load_state_dict(pretrained_weights)
    ############
    ...
    #
    # # 仅解冻 encodeA 部分
    # for param in self.model_rgb.encodeA.parameters():
    #   param.requires_grad = True
    #
    # parameters_to_train = list(filter(lambda p: p.requires_grad, self.model_rgb.parameters()))
    # self.optimizer = optim.Adam(parameters_to_train, weight_decay=0.00004, lr=0.00001)


    ############## sunhan  test
    self.model_rgb = RefineNet_RGB(cfg=self.cfg, c_in=4).cuda()
    # pretrained_weights = torch.load('/home/sunh/6D_ws/Fpose_rgb/weights/8.from_4080++/model_best_19.pth')
    pretrained_weights = torch.load('/home/sunh/6D_ws/Fpose_rgb/weights/9.效果更好的from_4080/model_best.pth')
    # pretrained_weights = torch.load('/home/sunh/6D_ws/Fpose_rgb/weights/10.29/model_best.pth')
    # pretrained_weights = torch.load('/home/sunh/6D_ws/Fpose_rgb/model_best.pth')
    self.model_rgb.load_state_dict(pretrained_weights['model_state_dict'])

    # pretrained_weights = torch.load(ckpt_dir)  ### ori weight
    # with torch.no_grad():
    #     pretrained_weights['encodeA.0.net.0.weight'] = pretrained_weights['encodeA.0.net.0.weight'][:,:3,:,:]
    # self.model_rgb.load_state_dict(pretrained_weights, strict=False)
    self.model_rgb.cuda().eval()
  ...
  @torch.inference_mode()
  def predict(self, rgb, depth, K, ob_in_cams, xyz_map, normal_map=None, get_vis=True, mesh=None, mesh_tensors=None, glctx=None, mesh_diameter=None, iteration=5):
    '''
    @rgb: np array (H,W,3)
...
#######################################################  zero shot rgb refine ################################################################
  def train_rgb_refine(self, rgb, depth, K, ob_in_cams, xyz_map, normal_map=None, get_vis=True, mesh=None, mesh_tensors=None, glctx=None, mesh_diameter=None, iteration=5):
    '''
    @rgb: np array (H,W,3)
    @ob_in_cams: np array (N,4,4)
    '''
    ...
    for _ in range(iteration):
      ...
      for b in range(0, pose_data.rgbAs.shape[0], bs):
        ...
        output_pre = self.model_rgb(A,B)

        loss_rot = F.mse_loss(output_pre['rot'], output['rot'])
        loss_trans = F.mse_loss(output_pre['trans'], output['trans'])
        self.loss = loss_rot + 10*loss_trans
        self.loss.backward()
        self.optimizer.step()
        ...
        if self.cfg['trans_rep'] == 'tracknet':
          ...
          uv_pred = transform_pts(uv_pred_crop, pose_data.tf_to_crops[b:b + bs].inverse().cuda())
          center_pred = torch.cat([uv_pred, torch.ones((len(rot_delta), 1), dtype=torch.float, device='cuda')], dim=-1)
          center_pred = (pose_data.Ks[b:b + bs].inverse().cuda() @ center_pred.reshape(len(rot_delta), 3, 1)).reshape(
  ...
  def predict_rgb(self, rgb, depth, K, ob_in_cams, xyz_map, normal_map=None, get_vis=True, mesh=None, mesh_tensors=None, glctx=None, mesh_diameter=None, iteration=5):
    '''
    @rgb: np array (H,W,3)
    @ob_in_cams: np array (N,4,4)
    '''
